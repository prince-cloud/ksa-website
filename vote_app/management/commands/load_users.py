...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--level",
            help="Specifies the Level of the students.",
        )
        parser.add_argument(
            "--election",
    ...
    def handle(self, *args, **kwargs):
        ...
        level = kwargs["level"]
        level = int(level)
        if not level in [100, 200, 300]:
            raise Exception(f"Unaccepted Level: {level}")
        time = timezone.now()
        ...
        with open(filename, "r") as f:
            ...
            for line in c:
                _, name, index, phone = line
                if User.objects.filter(username=index.strip()).exists():
                    print(f"Skipping user {name} with index {index} and phone {phone}")
                    continue
                ...
                try:
                    u.save()
                    message = f"Hello login to vote at https://elections.jotimall.com/ with your student index number as username and      password: {password}"
